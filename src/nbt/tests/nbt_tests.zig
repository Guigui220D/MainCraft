...
test "decode hello_world.nbt" {
    ...
    const hello_world_compound = nbt_root.compound.hashmap.get("hello world").?.tag_compound.hashmap;

    try std.testing.expectEqual(@as(usize, 1), hello_world_compound.count());
    try std.testing.expectEqualStrings("Bananrama", hello_world_compound.get("name").?.tag_string);
}
...
test "decode bigtest.nbt" {
    ...
    const level_compound = level.tag_compound.hashmap;
    try std.testing.expectEqual(11, level_compound.count());

    { // Check byteArray
        const byte_array = level_compound.get("byteArrayTest (the first 1000 values of (n*n*255+n*7)%100, starting with n=0 (0, 62, 34, 16, 8, ...))").?.tag_byte_array;
    ...
    { // Numbers
        const byte = level_compound.get("byteTest").?.tag_byte;
        try std.testing.expectEqual(@as(i8, 127), byte);

        const short = level_compound.get("shortTest").?.tag_short;
        try std.testing.expectEqual(@as(i16, 32767), short);

        const int = level_compound.get("intTest").?.tag_int;
        try std.testing.expectEqual(@as(i32, 2147483647), int);

        const long = level_compound.get("longTest").?.tag_long;
        try std.testing.expectEqual(@as(i64, 9223372036854775807), long);

        const float = level_compound.get("floatTest").?.tag_float;
        try std.testing.expectApproxEqAbs(@as(f32, 0.4982315), float, 0.01);

        const double = level_compound.get("doubleTest").?.tag_double;
        try std.testing.expectApproxEqAbs(@as(f64, 0.493128713218231), double, 0.0001);
    }

    { // String
        const string = level_compound.get("stringTest").?.tag_string;
        try std.testing.expectEqualStrings("HELLO WORLD THIS IS A TEST STRING ÅÄÖ!", string);
    }

    { // Lists
        const list_compound = level_compound.get("listTest (compound)").?.tag_list;
        const list_long = level_compound.get("listTest (long)").?.tag_list;
        ...
        try std.testing.expectEqualStrings("Compound tag #0", comp0.hashmap.get("name").?.tag_string);
        try std.testing.expectEqual(@as(i64, 1264099775885), comp0.hashmap.get("created-on").?.tag_long);

        try std.testing.expectEqualStrings("Compound tag #1", comp1.hashmap.get("name").?.tag_string);
        try std.testing.expectEqual(@as(i64, 1264099775885), comp1.hashmap.get("created-on").?.tag_long);
    }

    { // Nested compounds
        const nested_compound = level_compound.get("nested compound test").?.tag_compound;

        // Check length
        try std.testing.expectEqual(@as(usize, 2), nested_compound.hashmap.count());

        // Test contents
        const egg = nested_compound.hashmap.get("egg").?.tag_compound;
        const ham = nested_compound.hashmap.get("ham").?.tag_compound;

        try std.testing.expectEqualStrings("Eggbert", egg.hashmap.get("name").?.tag_string);
        try std.testing.expectEqual(@as(f32, 0.5), egg.hashmap.get("value").?.tag_float);
